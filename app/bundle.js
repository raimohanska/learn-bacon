...
Bacon.Observable.prototype.withTimestamp = function(_arg) {
  var offset, precision, relative, _ref;
  _ref = _arg != null ? _arg : {
    precision: 1
  }, relative = _ref.relative, precision = _ref.precision;
  offset = relative ? new Date().getTime() : 0;
  return this.flatMap(function(value) {
    return {
      value: value,
      timestamp: Math.floor((new Date().getTime() - offset) / precision)
    };
  });
};
...
evaluateAssignment = function(assignment, code) {
  var actual, expected, expectedValues, values;
  actual = evalCode(code).apply(null, assignment.inputs());
  expected = evalCode(assignment.example).apply(null, assignment.inputs());
  values = [];
  expectedValues = [];
  collectAndVisualize(actual, values, "expected");
  return collectAndVisualize(expected, expectedValues, "actual");
};

collectAndVisualize = function(src, values, desc) {
  return src.withTimestamp({
    relative: true,
    precision: 100
  }).onValue(function(value) {
    values.push(value);
    return console.log(desc, value);
  });
};
