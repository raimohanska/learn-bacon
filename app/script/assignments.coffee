module.exports = [
  ...
  }
  {
    ...
    example: "return stream.map(function(a) { return a + 1})"
    inputs: -> [Bacon.sequentially(1000, [1,2,3]).name("stream")]
    tip: "map"
  }
  {
    description: "Given a stream of integers, filter out those less than 2"
    example: "return stream.filter(function(a) { return a >= 2 })"
    inputs: -> [Bacon.sequentially(1000, [1,2,3,1]).name("stream")]
    tip: "filter"
  }
  {
    description: "Given a stream of functions, call each one with the value 1."
    example: "return stream.map(function(f) { return f(1) })"
    inputs: -> [Bacon.sequentially(1000, [((x) -> x),((x) ->  x+1)]).name("stream")]
    tip: "map"
  }
  {
    description: "Given a stream of integers, calculate a running sum of the values."
    example: "return stream.scan(0, function(acc, value) { return acc + value})"
    inputs: -> [Bacon.sequentially(1000, [1,2,3]).name("stream")]
    tip: "scan"
  }
  {
    description: "Delay stream values by 500 milliseconds"
    example: "return stream.delay(500)"
    inputs: -> [Bacon.sequentially(1000, [1,2,3]).name("stream")]
    tip: "delay"
  }
  {
    description: "Output all values, but limit output to maximum 1 event per 1000 milliseconds",
    example: "return stream.bufferingThrottle(1000)"
    inputs: -> [Bacon.sequentially(500, [1,2,3]).name("stream")]
